...
async function submitUpload() {
  ...
  const error = validateFile(uploadFile.value)
  if (error) {
    uploadError.value = error
    ElMessage.warning(error)
    return
    ...
    <section class="upload-strip soft-card">
      <div class="soft-card-body upload-inner">
        ...
        <el-upload ref="uploadRef" drag action="#" :auto-upload="false" class="upload-box" :limit="1" :on-change="onFileChange">
          ...
          <div class="el-upload__text">{{ uploadFile?.name || '拖拽文件到这里，或点击选择' }}</div>
          <template #tip><div class="el-upload__tip">支持 PDF / DOCX / TXT / MD，单文件 20MB 内</div></template>
        </el-upload>
      </div>
    </section>
        ...
        <el-table :data="docs" v-loading="loading" size="large" empty-text="暂无文档">
          ...
          <el-table-column label="操作" width="230" fixed="right">
            <template #default="{ row }">
              ...
              <el-button v-if="row.parseStatus === 'FAILED'" link type="warning" @click="retry(row)">重试</el-button>
