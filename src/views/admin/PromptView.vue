...
<template>
  <div>
    <div class="page-header"><div><h1 class="page-title">Prompt 模板</h1><div class="page-desc">管理问答和摘要场景的 Prompt 模板，支持新增、编辑、删除和启用。</div></div><el-button type="primary" @click="openCreate">新增模板</el-button></div>
    <section class="soft-card"><div class="soft-card-body">
      <el-table :data="prompts" v-loading="loading" size="large" empty-text="暂无 Prompt 模板">
        <el-table-column prop="name" label="模板名称" min-width="240" /><el-table-column prop="type" label="类型" width="120" /><el-table-column label="更新时间" width="170"><template #default="{ row }">{{ timeOf(row) }}</template></el-table-column>
    ...
    <el-drawer v-model="drawerVisible" :title="editingId ? '编辑 Prompt' : '新增 Prompt'" size="560px">
      <el-form label-position="top"><el-form-item label="模板名称"><el-input v-model="form.name" /></el-form-item><el-form-item label="模板类型"><el-select v-model="form.type" style="width: 100%"><el-option label="问答" value="问答" /><el-option label="摘要" value="摘要" /><el-option label="关键词提取" value="关键词提取" /></el-select></el-form-item><el-form-item label="模板内容"><el-input v-model="form.content" type="textarea" :rows="9" /></el-form-item><el-form-item label="启用状态"><el-switch v-model="form.enabled" /></el-form-item></el-form>
      <template #footer><el-button @click="drawerVisible = false">取消</el-button><el-button type="primary" @click="save">保存</el-button></template>
    </el-drawer>
  </div>
</template>
