...
import { docCountOf, textOf, timeOf } from '@/utils/view-adapters'
...
async function toggleStatus(row: KnowledgeBase) {
  ...
  await ElMessageBox.confirm(`确认${action}知识库「${textOf(row.name)}」吗？`, `${action}确认`, { type: next === 'DISABLED' ? 'warning' : 'info' })
...
async function remove(row: KnowledgeBase) {
  await ElMessageBox.confirm(`确认删除知识库「${textOf(row.name)}」吗？该操作会影响关联文档和问答记录。`, '删除确认', { type: 'warning' })
...
<template>
  <div>
    <div class="page-header">
      <div>
        <h1 class="page-title">后台知识库管理</h1>
        <div class="page-desc">管理员查看全平台知识库，可禁用、启用和删除数据。</div>
      </div>
    ...
    <section class="soft-card">
      <div class="soft-card-body">
        <el-table :data="list" v-loading="loading" size="large" empty-text="暂无知识库">
          <el-table-column label="知识库名称" min-width="220" show-overflow-tooltip>
            <template #default="{ row }">{{ textOf(row?.name) }}</template>
          </el-table-column>
          <el-table-column label="分类" width="120">
            <template #default="{ row }">{{ textOf(row?.category) }}</template>
          </el-table-column>
          <el-table-column label="文档数量" width="100">
            <template #default="{ row }">{{ docCountOf(row) }}</template>
          </el-table-column>
          ...
          <el-table-column label="状态" width="120">
            <template #default="{ row }">
              <el-tag :type="row.status === 'NORMAL' ? 'success' : row.status === 'FAILED' ? 'danger' : row.status === 'DISABLED' ? 'info' : 'warning'">
                {{ textOf(row?.status) }}
              </el-tag>
            </template>
          </el-table-column>
