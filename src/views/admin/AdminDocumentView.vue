...
import { documentErrorOf, documentNameOf, fileTypeOf, kbNameOf, statusTagType, textOf, timeOf } from '@/utils/view-adapters'
      ...
      <div class="soft-card-body">
        <el-table :data="docs" v-loading="loading" size="large" empty-text="暂无文档">
          ...
          <el-table-column label="解析状态" width="120">
            <template #default="{ row }"><el-tag :type="statusTagType(row?.parseStatus)">{{ textOf(row?.parseStatus) }}</el-tag></template>
          </el-table-column>
          <el-table-column label="向量状态" width="120">
            <template #default="{ row }"><el-tag :type="statusTagType(row?.embeddingStatus)" effect="plain">{{ textOf(row?.embeddingStatus) }}</el-tag></template>
          </el-table-column>
